...
const handleStop = async () => {
    if (language === "am") {
        ...
        const pusem=getGamePusedAudiom();
        pusem.play();}
     else {
...
useEffect(() => {
       ...
       const newgamet = useCallback(async () => {
           ...
           try {
               ...
               if (language === "am") {
                   ...
                   const startf = getGameStartedAudim();
                   await startf.play();
                   await new Promise(resolve => { startf.onended = resolve; });
               } 
               else {
  ...
  //  console.log("the value of game start is ",gamestart)
    ...
    const getGameStartedAudif = () => {
     ...
     const getGameStartedAudim = () => {
        const audio = new Audio();
        audio.src = `/gamestatusmale/gamestarted.mp3`;
        audio.load(); // Ensures the browser loads it properly
        return audio;
                ...
                const playAmharicAudio = (number) => {
                    return new Promise((resolve) => {
                        if (language === "am") {
                            ...
                            playAmharicAudioForNumbermale(number);
                        }
...
const shuffleArray = (array) => {
 ...
 const playAmharicAudioForNumbermale = (number) => {
    if (number >= 1 && number <= 75) {
        try {
